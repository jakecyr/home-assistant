...
export interface BeepTimerArgs {
  hours?: number;
  minutes?: number;
  seconds?: number;
  label?: string;
...
export class BeepTimerTool {
  readonly name = "timer_beep";
  readonly description =
    "Start a countdown timer by providing hours, minutes, or seconds. Plays a 5-second beep tone when finished.";
  ...
  async exec(rawArgs: BeepTimerArgs): Promise<ToolExecutionResult> {
    ...
    const messageParts = [
      ...
      "It will play a 5-second beep when finished.",
    ].filter(Boolean);
...
function normalizeParts(args: BeepTimerArgs) {
  return {
